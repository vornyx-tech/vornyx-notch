...
# Sign with the certificate from Configuration/setup_local_signing.sh when it
# is there: an ad-hoc signature is the build's own hash, so every DMG would be
# a new app to macOS and lose its Accessibility grant (HUD, Focus light).
SIGN_IDENTITY="-"
SIGN_NAME="ad-hoc signed"
if security find-identity -v -p codesigning 2>/dev/null | grep -q '"Vornyx Notch Local Signing"'; then
  SIGN_IDENTITY="Vornyx Notch Local Signing"
  SIGN_NAME="signed with the local certificate"
fi

mkdir -p "$BUILD_DIR"
rm -rf "$ARCHIVE" "$APP"

echo "==> Archiving $PROJECT_NAME $VERSION (Release, universal, $SIGN_NAME)"
