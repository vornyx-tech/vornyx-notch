...
/// Generous, so the notch opens on the way up rather than at the very top
/// edge, where an overshooting drag opens Mission Control instead.
let dragOpenSlack = CGSize(width: 120, height: 90)
...
enum WeatherGlowStyle {
    ...
    /// How far in from the page's own edges the glow has faded to nothing.
    /// Bled past the frame instead of just enlarging this: with the column's
    /// clip exact on three sides, spilling past made the glow read as a
    /// bigger, stronger wash that bled into the forecast card and whatever
    /// sat behind the window, worse than the hard edge it replaced. A softer
    /// taper fully inside the frame is the fix; this only needed to grow a
    /// little from the original 28 to read as gradual rather than abrupt.
    static let edgeFade: CGFloat = 40
