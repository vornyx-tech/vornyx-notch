...
struct ContentView: View {
                ...
                mainLayout
                    .conditionalModifier(true) { view in
                        ...
                        return view
                            ...
                            // The closed notch widening for the music live
                            // activity coming back: nothing above keys on
                            // this, so without it the black shape's own width
                            // change had no animation to ride and could
                            // settle a frame or two apart from the edge light
                            // drawn on top of it, showing as a stroke that
                            // did not quite hug the black.
                            .animation(notchResizeAnimation, value: musicManager.isPlaying)
                            .animation(notchResizeAnimation, value: musicManager.isPlayerIdle)
    ...
    /// long as a transfer or pinned files need it.
    ...
    private func doOpen() {
        // Same rule as hover: nothing is set up yet during onboarding, and a
        // click on the closed notch used to open it anyway, straight into
        // empty tabs - hover was the only path that already guarded against
        // this.
        ...
        if hovering {
            ...
            hoverTask = Task {
                ...
                await MainActor.run {
                    ...
                    // A button held down means something is being dragged in,
                    // and a dragged file belongs on the shelf. Hover can get
                    // here before the drop target does, which then sees the
                    // notch already open and leaves it on whatever page it was.
                    if NSEvent.pressedMouseButtons & 1 != 0, Defaults[.shelfEnabled] {
