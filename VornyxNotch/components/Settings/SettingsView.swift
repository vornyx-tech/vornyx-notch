        ...
        Form {
            ...
            Section {
                ...
                Defaults.Toggle(key: .showFocusIndicator) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Focus light")
                        Text("Needs Accessibility, Focus set to show in the menu bar \"When Active\", and the menu bar itself not set to auto-hide.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        FocusLightStatus()
                    }
...
struct Appearance: View {
    ...
    @Default(.hudMatchesAlbumArt) var hudMatchesAlbumArt
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Defaults.Toggle(key: .hudMatchesAlbumArt) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Volume and brightness in album colour")
                        Text("While something with real artwork is playing, the HUD bar and its icon pick up the cover's colour instead of \(sliderColor == .accent ? "the accent colour" : "white").")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
                .disabled(sliderColor != .albumArt)
            } header: {
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text(
                    "Open Shelf opens the notch on the shelf, and closes it if the shelf is already showing. Show Clipboard opens the notch on the clipboard tab. Unset by default - record a shortcut to use it, and turn the clipboard on under Clipboard first."
                )
                .multilineTextAlignment(.trailing)
                .foregroundStyle(.secondary)
                .font(.caption)
...
/// What the Focus light can actually see right now, with the fix one click
/// away - the light has no other way to say why it never comes on.
private struct FocusLightStatus: View {
    @ObservedObject private var indicators = IndicatorsManager.shared
    @Default(.showFocusIndicator) private var enabled
    ...
    private var symbol: String {
        switch indicators.focusStatus {
        case .on, .off: return "checkmark.circle.fill"
        case .noAccessibility, .noMenuBar: return "exclamationmark.triangle.fill"
    ...
    private var tint: Color {
        switch indicators.focusStatus {
        case .on, .off: return .green
        case .noAccessibility, .noMenuBar: return .orange
    ...
    private var message: String {
        switch indicators.focusStatus {
        case .on: return "Working - a Focus is on."
        case .off: return "Working - no Focus is on."
        case .noAccessibility: return "This build has no Accessibility access. Each new build needs it granted again."
        case .noMenuBar: return "Control Center's menu bar can't be read."
        case nil: return "Checking..."
        }
    }

    private var fix: (title: String, action: () -> Void)? {
        switch indicators.focusStatus {
