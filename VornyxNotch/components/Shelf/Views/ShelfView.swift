...
struct ShelfView: View {
    ...
    var body: some View {
        ...
        // Space toggles the preview for whatever is selected, the same as
        // Finder. The context menu's own "Quick Look" item opens the same
        // panel; this is just the keyboard route to it.
        .onKeyDown(handleSpaceKey)
    }

    private func handleSpaceKey(_ event: NSEvent) -> Bool {
        guard NotchKey(event) == .space else { return false }
