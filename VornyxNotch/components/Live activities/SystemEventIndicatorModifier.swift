...
/// The HUD's colour when the album art is standing in for white/accent:
/// `nil` outside that condition, so every caller falls back to what it
/// already drew, unchanged.
enum HUDAccentColor {
    static var current: Color? {
        guard Defaults[.hudMatchesAlbumArt], Defaults[.sliderColor] == .albumArt else { return nil }
        let music = MusicManager.shared
        guard music.isPlaying, !music.usingAppIconForArtwork else { return nil }
        return Color(nsColor: music.avgColor).ensureMinimumBrightness(factor: 0.7)
    }
}
        ...
        VStack {
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        .fill(
                            HUDAccentColor.current.map(AnyShapeStyle.init) ??
