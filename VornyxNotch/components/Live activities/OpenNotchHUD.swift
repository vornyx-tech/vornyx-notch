...
struct OpenNotchHUD: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            // Some device glyphs (AirPods among them) default to hierarchical
            // shading, which ignores a plain foregroundStyle color; forced to
            // one flat tint so the album-colour setting actually shows on them.
            .symbolRenderingMode(.monochrome)
            .foregroundStyle(HUDAccentColor.current ?? .white)
