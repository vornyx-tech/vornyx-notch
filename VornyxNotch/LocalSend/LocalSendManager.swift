...
final class LocalSendManager: ObservableObject {
    ...
    /// Files dropped on the strip and waiting for a device and Send. Kept here
    /// rather than in the view, so they survive the notch closing.
    @Published private(set) var staged: [URL] = []
    /// A pinned file being dragged back out, towards the trash. Its drag is not
    /// a new drop, so the strip keeps its trash showing instead of turning into
    /// the drop area.
    @Published var draggingStaged: URL?
    ...
    /// Pin files to send later. The same file twice is pinned once.
    func stage(_ urls: [URL]) {
        let fresh = urls.filter { url in
            !staged.contains { $0.standardizedFileURL == url.standardizedFileURL }
        }
        guard !fresh.isEmpty else { return }
        withAnimation(.smooth(duration: 0.25)) { staged.append(contentsOf: fresh) }
    }

    func unstage(_ url: URL) {
        withAnimation(.smooth(duration: 0.25)) {
            staged.removeAll { $0.standardizedFileURL == url.standardizedFileURL }
        }
    }

    func clearStaged() {
        withAnimation(.smooth(duration: 0.25)) { staged.removeAll() }
    }

    /// Send everything pinned, then let it go.
    func sendStaged(to device: LocalSendDevice) {
        // A send still running would ignore these, and they would be lost.
        guard !staged.isEmpty, outgoing.map(\.isOver) ?? true else { return }
        let urls = staged
        send(urls, to: device)
        clearStaged()
    }
