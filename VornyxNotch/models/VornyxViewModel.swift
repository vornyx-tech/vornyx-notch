...
class VornyxViewModel: NSObject, ObservableObject {
    @ObservedObject var coordinator = VornyxViewCoordinator.shared
    @ObservedObject var detector = FullscreenMediaDetector.shared

    let animationLibrary: VornyxAnimations = .init()
    ...
    /// Drives the mirror from the camera tab appearing/disappearing rather than
    /// from a toggle press. Falls back to `toggleCameraPreview()` when access has
    /// not been granted yet, so the permission prompt still happens.
    func setCameraPreview(active: Bool) {
        guard webcamManager.authorizationStatus == .authorized else {
            if active { toggleCameraPreview() }
            return
        }
