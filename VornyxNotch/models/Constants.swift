...
extension Defaults.Keys {
    ...
    /// A player card over the login screen while something is playing.
    static let lockScreenPlayer = Key<Bool>("lockScreenPlayer", default: true)
    ...
    /// Carries the album art colour (already on for the player's own slider,
    /// via `sliderColor`) over to the volume/brightness HUD and its icon,
    /// while something with real artwork is playing. Plain white/accent the
    /// rest of the time, same as today.
    static let hudMatchesAlbumArt = Key<Bool>("hudMatchesAlbumArt", default: true)
    ...
    static let waitInterval = Key<Double>("waitInterval", default: 5)
