...
final class YouTubeMusicController: MediaControllerProtocol {
    ...
    deinit {
        ...
        appStateObservers.forEach(center.removeObserver)
    }
    ...
    /// Block observers on the main queue, not `notifications(named:)` in a
    /// task group: that read `NSWorkspace.shared.notificationCenter` off the
    /// main thread from a child task, which traps at runtime. Only the bundle
    /// identifier leaves the closure - a Notification is not Sendable.
    private func setupAppStateObserver() {
        ...
        appStateObservers = [
            center.addObserver(
                ...
                Task { await self?.handleAppLaunched(bundleID: bundleID) }
            },
            center.addObserver(
                ...
                let bundleID = (notification.userInfo?[NSWorkspace.applicationUserInfoKey]
                    as? NSRunningApplication)?.bundleIdentifier
                Task { await self?.handleAppTerminated(bundleID: bundleID) }
            },
        ]
    }
    ...
    private func handleAppTerminated(bundleID: String?) async {
        ...
        // The observers stay: they are what notices the app coming back.
        Task { @MainActor in
            stopPeriodicUpdates()
        }
